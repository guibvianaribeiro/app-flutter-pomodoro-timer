...
class PomodoroController extends StateNotifier<PomodoroState> {
  ...
  void startFocus() =>
      _startPhase(PomodoroPhase.focus, state.config.focusMinutes * 60);
  void startShortBreak() => _startPhase(
      PomodoroPhase.shortBreak, state.config.shortBreakMinutes * 60);
  void startLongBreak() =>
      _startPhase(PomodoroPhase.longBreak, state.config.longBreakMinutes * 60);
